...
## Summary
  ...
  Resolves the **high-severity** Dependabot alert **CVE-2026-56876 /
  GHSA-jmr9-qjv8-65gv** — `extract-zip` unvalidated symlink path traversal
  (CWE-22, CVSS 8.1).

  `extract-zip@2.0.1` entered the tree **dev-only** and transitively:
  `puppeteer → @puppeteer/browsers@2.13.2 → extract-zip`. It is
  **unmaintained** (last release 2023) and the advisory covers `<=2.0.1`
  with **no patched version**, so the usual `overrides` pin has nothing
  safe to target. npm's remedy is a top-level bump: `puppeteer@25.6.0`,
  whose `@puppeteer/browsers@3.2.0` dropped extract-zip in favour of the
  actively-maintained, zero-dependency `modern-tar`.

  ## Change

  - `devDependencies.puppeteer`: `^24.43.1` → `^25.6.0`
  - Lockfile regenerated: `@puppeteer/browsers` 2.13.2 → 3.2.0;
  **`extract-zip` fully removed** (2 → 0 lockfile entries).

  ## Verification

  - `npm audit` → **0 vulnerabilities** (clears this advisory and leaves
  the tree clean).
  - An adversarial verification pass (fix-chain, breakage,
  CI/licence-impact, reachability, plus a skeptic) confirmed the fix is
  complete and safe. Highlights:
  - **No code changes needed.** puppeteer is used only by three
  manually-run dev scripts (`generate-icons`, `svg-to-apng`,
  `visual-regression`) — never by the app, the vitest suite, or any CI
  workflow. They use only the stable
  `launch/newPage/goto/screenshot/close` surface, unchanged in v25.
  - **Never shipped / never reachable** from the signed app — extract-zip
  was dev-tooling only; even pre-fix the vuln could not reach end users.
  - **No CI/licence-gate impact** — the licence checks are
  runtime-dep-only; no new direct dependency is introduced.

  ## Scope of changes

  - [x] Dependencies (`package.json` / `package-lock.json`)
  - [ ] Rust / Frontend / IPC / Settings — none

  ## Security review

  - [x] No secrets, no subprocess/shell changes, no `pull_request_target`.
  - [x] Swapped-in `modern-tar` is maintained (latest release days old),
  MIT, **zero transitive deps** — a strictly better supply-chain position
  than the unmaintained extract-zip it replaces.

  ## Note

  One local-run caveat for contributors: puppeteer 25.x requires **Node ≥
  22.12.0** (was ≥18) to run those three dev scripts. Not a CI concern
  (runners use `lts/*`).
  ...
  Code](https://claude.ai/code/session_01UXLvhP2QYN4yBHwmBwRRUB)_
...
- Update CHANGELOG.md [skip ci]
- Update CHANGELOG.md [skip ci]
- Add August 2026 dependency staleness audit (#1090)

## Summary

  Adds `.github/audits/dependency-staleness-2026-08.md` — a proactive
  follow-up to the extract-zip CVE. It sweeps the full transitive tree for
  the *unmaintained-leaf* risk class Dependabot cannot catch (a stale
  dependency with no upgrade to offer, which is exactly how extract-zip
  slipped through).

  ## Method & verdict

  - Scanned **1,336 packages** (539 npm + 797 Rust), flagged **242** as ≥2
  years stale / deprecated, then triaged each by *function × maintenance ×
  reachability* with per-candidate deep-dives and an adversarial
  completeness critic.
  - **Verdict: no "next extract-zip" in the tree.** 0 urgent actions; 1
  `monitor` (`lzma-sys`, well-mitigated); 225 stale-but-stable micro-libs.

  The doc records the watch list, exclusion evidence (e.g. the `actix-*`
  crates are phantom — never compiled), optional hygiene notes, and a
  quarterly re-run cadence.

  ## Scope of changes

  - [x] Docs only (`.github/audits/`)
  - [ ] No code / dependency / workflow changes
...
- Auto-merge green Dependabot PRs (alpha + security→main), 7-day cooldown, deploy forward-port to main (#1087)

## Summary

  Implements the maintainer-approved dependency-flow policy (#1086):
  `alpha` gets dependencies + security patches first, and green Dependabot
  PRs merge themselves. Three parts — a new auto-merge workflow, a 7-day
  Dependabot cooldown, and deploying the existing security forward-port
  workflow to `main` so it can actually fire.
