...
impl DownloadIndex {
    ...
    fn migrate(&mut self) -> Result<(), IndexError> {
        ...
        // Future: if current < 2 { self.apply_schema_v2()?; } …
...
mod tests {
    ...
    fn refuses_to_open_schema_newer_than_known() {
        ...
        match err {
            IndexError::SchemaTooNew { found: 99, known: 1 } => {}
    ...
    fn schema_creates_every_required_table() {
        ...
        let tables: Vec<String> = idx
            ...
            .collect();
        // 7 user tables: downloads, recordings, recording_downloads,
        // manifests, activity_events, known_tracks, schema_version.
        for required in [
            "downloads",
            "recordings",
            "recording_downloads",
