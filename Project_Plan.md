...
**v0.39.0** (2026-03-30) — All 6 phases complete + post-release features <!-- x-release-please-version -->
...
- ✅ **Release channel ladder + nightly auto-release** - Six-tier channel hierarchy `feat/* → nightly → weekly → monthly → alpha → beta → main (stable)` with protected long-lived branches. `nightly-release.yml` merges `feat/*` into `nightly` daily at 00:00 UTC, bumps to `-nightly.YYYYMMDD`, and pushes a tag to trigger `release.yml`. `UpdateChannel` enum + `update_channel` AppSetting with channel-aware update checker and install guard (`download_and_install_app_update` refuses tags from a less-stable channel). Settings > General > Updates channel dropdown. `.github/rulesets/protected-release-branches.json` + `apply-branch-rulesets.yml` block deletion/non-FF on `main`/`beta`/`alpha`/`monthly`/`weekly`/`nightly`. `auto-delete-merged-branches.yml` prunes merged PR head branches except the protected channels. Weekly/monthly cadences to follow the same template (crons `0 0 * * 0` and `0 0 1 * *`).
