...
- ✅ **Remote service status (kill switch)** - Developers can remotely enable/disable media services via `service-status.json` committed to the `main` branch. App checks on startup and every 4 hours from `raw.githubusercontent.com`. Fail-open design: network failure → cached config → all-enabled default. Disabled services show warning banners, block new downloads, and disable the "Add to Queue" button. Service-specific messages explain the reason. `Dev_Notes.md` documents how to use the system.
- ✅ **Smart Download infrastructure (cross-platform quality comparison)** - Phase 1 of cross-platform quality optimization. `QualityTier` enum normalizes codec/quality settings across services (Lossy128 → DolbyAtmos). `get_service_quality_tier()` maps each service's configured settings to a quality tier. Collapsible Smart Download panel in DownloadForm shows quality tier for the detected service. Toggle in Settings > General (`smart_download_enabled`, default: off). Future phases will add cross-platform ISRC/UPC search to find best quality across all enabled services.
