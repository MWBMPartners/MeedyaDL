...
- ✅ **Metadata enrichment** - Comprehensive post-download enrichment: codec tags, source/channel tags, Apple Music API metadata (ISRC, UPC, genre, advisory, artist IDs, artwork URLs), song.link cross-platform links. Shared `apple_music_api.rs` module for MusicKit JWT, URL parsing, and catalog API.
...
- ✅ **MusicBrainz recording lookup** (v0.6.4; module split + guarded search fallback added for 2026-11-30 search-upgrade readiness) - Enrichment Step 6b discovers music videos and cross-platform URLs via the MusicBrainz database, now split into `services/musicbrainz_service/` (`mod.rs` + `relations.rs` + `search.rs`). Identifier chain, always tried first: (T1) Apple Music URL lookup via MB's URL browse endpoint (latent), (T2) ISRC lookup via the dedicated `/isrc` endpoint (the live path), (T3) MusicBrainz recording ID direct lookup from AcoustID (latent). When all three miss, a guarded search fallback (S1 recording search + S2 once-per-album wildcard URL search, both identifier-discovery only — every relationship is still read back out of lookup/browse, never out of a search response) resolves further tracks, gated by the new `musicbrainz_search_fallback` setting (default: true) and an advisory-only Solr-version probe that only affects request economy, never parsing. No credentials required (free public API). Rate-limited to 1 req/sec via a single process-global limiter. Returns all discovered platform URLs (Apple Music, YouTube, Spotify, Deezer, Tidal, SoundCloud, Bandcamp) for future cross-platform song discovery — logged for diagnostics today, persisting them is still planned. Storefront-aware URL rewriting (implemented, not yet wired). `musicbrainz_lookup` setting (default: false) gates the whole feature; `musicbrainz_search_fallback` (default: true) gates just the search tier. Toggle in Settings > Codec & Resolution > Video Quality.
...
- ✅ **Fallback mirror for tool downloads** - When primary upstream sources fail, falls back to `MeedyaSuite/MeedyaDL-Tools` GitHub Releases with standardized asset naming (`{tool_id}-{os}-{arch}.{ext}`)
...
- ✅ **Crash report improvements** - `delete_all_crash_reports` command and "Clear All" UI button in Settings > Advanced > Error Reporting
