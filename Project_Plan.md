...
**v0.6.3** (2026-03-01) — All 6 phases complete + post-release features <!-- x-release-please-version -->
...
- ✅ **Graceful shutdown for background tasks** - `ShutdownSignal` (`Arc<AtomicBool>`) registered as Tauri managed state. Triggered on window close (`WindowEvent::Destroyed`) and tray quit. Checked at iteration boundaries in enrichment pipeline (5 stages), companion download loops (tiers), and lyrics companion loops (formats). Fire-and-forget tasks exit early instead of starting new work
