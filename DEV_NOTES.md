...
### `ACKNOWLEDGEMENTS.md` — Maintained By Hand

Lists all currently enabled/shipping dependencies with licence links, versions, and one-line purposes. **This file is maintained by hand, not generated** — matching what [THIRD_PARTY_LICENSES.md](THIRD_PARTY_LICENSES.md) says about itself ("generated by hand and reviewed at each release"). Update it directly whenever a dependency, engine, or tool is added, removed, or re-licensed upstream.

`ACKNOWLEDGEMENTS.md` is maintained by hand, and that is deliberate.

There used to be a `scripts/generate-acknowledgements.mjs` that built it
from the dependency files. It was deleted on 2026-09-10. Two reasons. It
produced a much thinner file — no version column, none of the notes
explaining what a licence means for somebody using the app — and it
carried a hand-written licence table of its own that was wrong: it called
mp4decrypt/Bento4 MIT when it is GPL-2.0 with a linking exception, and it
listed two libraries as direct dependencies when they arrive through
another one.

So running it would have quietly replaced correct legal information with
incorrect legal information. A script nobody may safely run is not a tool,
it is a trap, so it is gone rather than left with a warning on it. If a
generator is wanted again, it has to produce the same detail and be
checked against the upstream licences the same way `npm run check:legal`
checks the hand-maintained file today.

Two scripts DO run automatically in CI and are safe (they only check, never write): `node scripts/check-acknowledgements.mjs` confirms every direct dependency is named in the file, and `node scripts/check-upstream-licences.mjs` confirms the licence string named for each matches what upstream actually declares. Run both via `npm run check:legal`.
...
This covers `MWBMPartners/MeedyaSuite-core` (shared Rust crates), `MeedyaSuite/MeedyaDL-Tools` (dependency mirrors), and any future repos under either org — regardless of branch, tag, or rev qualifiers. (This section previously named the second org `MeedyaDL` instead of `MeedyaSuite` — a typo that didn't match the actual `deny.toml`, and didn't even match the very next sentence's own example, `MeedyaSuite/MeedyaDL-Tools`.)
