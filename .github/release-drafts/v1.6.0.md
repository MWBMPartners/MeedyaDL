...
>
> **`{platform}` template variable is broken (#829).** v1.6.0 was the first release to add the `[Platform]` chip to every Folder Template and File Template builder by default (#800). If your Settings → Templates contain that chip, **every download will fail** with `Error processing "...": 'platform'`. The underlying cause is a Python `KeyError: 'platform'` raised by GAMDL because MeedyaDL never substitutes `{platform}` to the service name before writing the template to `config.ini`. The variable was added to the frontend registry in v1.0.x (#309), but the corresponding backend substitution was never implemented.
>
...
>
> The proper fix lands in **v1.8.1**.
