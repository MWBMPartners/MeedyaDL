...
- **Batch C — plan ready, implementing next** (#963/#1002/#965 version-gate wrapper-dependency for GAMDL ≥3.8 via new `GamdlFeature::AssetsApiUnlocksLossyCodecs` + `is_wrapper_dependent_runtime()`, KEEP "(Experimental)" labels unconditional, version-conditional prose via a new `useGamdlCapabilities` hook + DTO field; #1000 REMOVE dead `fetch_extra_tags` plumbing — v2 preview-clip Sound-Check atoms, superseded by ReplayGain + dual-API enrichment; #1014 per-platform ceiling override `[gamdl.platform_ceilings]` ARMv7→3.8.1; #1001 wrapper-aware `recommended_upgrade_target()` v2→v3 migration). #1002 stays open pending live QA; the rest close with the batch.
