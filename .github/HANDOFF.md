...
- **NOT DONE — `download_queue.rs` submodule split.** Despite an earlier task marking, the split did **not** land: `src-tauri/src/services/download_queue.rs` is still a single monolithic file (~17.3k lines / 826 KB) with no split commit on this branch. Deliberately deferred out of this docs/version commit — it is a large, high-risk mechanical refactor that warrants its own focused, fully-`cargo test`-validated change rather than being smuggled into a documentation bump. Genuinely still pending; flag to the maintainer.
