...
**DEFERRED — Phase 2b+ (design §5):** PM-native staleness queries (`brew outdated --json`); venv-liveness detection + guided re-provision after a `brew upgrade python` breaks the venv symlink; opt-in external-GAMDL consumption (recommended: never); winget/choco attribution spike; yt-dlp detect-inform (fold into M8/M10 engine-setup UI). D5 (Scoop) is code-complete but needs a real-Windows validation pass before stable.

**Branch state.** `claude/gamdl-v3-8-5-review-gs36zl` HEAD advances with the Phase-2a commit + this handoff, pushed, == remote. Continues the one-branch model ahead of `alpha` (#1081 + the status-time follow-up + Phase 2a) — folds into the next single PR to `alpha`. `beta`/`release-candidate` still behind on the 2026-08-05 dependency-security fixes (see below) — `forward-port-security.yml` handles those independently.
