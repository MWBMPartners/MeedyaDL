...
6. **Trap:** `src/components/help/HelpViewer.tsx` does **not** read `help/*.md` at runtime — help content is inline template literals in a `HELP_TOPICS` array (`src/components/help/HelpViewer.tsx:170`). Any help-doc change must land in **both** the `.md` file and the matching inline `HELP_TOPICS` entry, or the two silently diverge. [**No longer true, 2026-09-09 (#1075):** the second copy is gone. The Help screen is now built from `help/*.md`, so a help change is one file edit. `src/components/help/helpTopics.ts` holds the list of pages (label, icon, order) and loads the files; `HelpViewer.tsx` only renders. `tools/audit-checks/check_help_topics.py` reports any way the two can disagree. Left in place rather than deleted so that anyone reading an older note about "the twin" can see when and why it stopped applying.]
