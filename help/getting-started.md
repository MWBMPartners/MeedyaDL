...
This guide walks you through the first-time setup of gamdl-GUI, from system requirements to your first download.
...
gamdl-GUI is available on the following platforms:
...
gamdl-GUI has **no special software prerequisites**. All dependencies -- including Python, GAMDL, FFmpeg, mp4decrypt, N_m3u8DL-RE, and MP4Box -- are automatically downloaded and installed into the application's sandboxed data directory on first launch. You do not need to install any of these tools yourself.

The only things you need before using gamdl-GUI are:
...
3. Drag the **gamdl-GUI** application into your **Applications** folder.
4. Eject the `.dmg` disk image.
5. Open gamdl-GUI from your Applications folder.

**Gatekeeper warning:** Because gamdl-GUI is not signed with an Apple Developer certificate, macOS may block the application the first time you open it. To bypass this:

- **Right-click** (or Control-click) the gamdl-GUI application and select **Open** from the context menu, then click **Open** in the confirmation dialog.
- Alternatively, go to **System Settings > Privacy & Security**, scroll down, and click **Allow** next to the message about gamdl-GUI being blocked.
...
3. Launch gamdl-GUI from the Start Menu or desktop shortcut.
...
2. Install it using your package manager or by running `sudo dpkg -i gamdl-gui_*.deb` in a terminal.
3. Launch gamdl-GUI from your application menu or by running `gamdl-gui` in a terminal.

**AppImage:**

1. Download the `.AppImage` file.
2. Make it executable: `chmod +x gamdl-GUI_*.AppImage`
3. Run it: `./gamdl-GUI_*.AppImage`
...
The first time you launch gamdl-GUI, a setup wizard will guide you through the initial configuration. The wizard consists of **6 steps**:
...
6. **Complete** -- Setup is finished and gamdl-GUI is ready to use.

The wizard handles everything automatically. Simply follow the prompts and wait for each step to complete.

### Setting Up Cookies

Before you can download any content, you must provide your Apple Music cookies. These cookies authenticate gamdl-GUI with Apple Music on your behalf.
...
By default, gamdl-GUI saves downloaded files to your system's default music directory (e.g., `~/Music` on macOS and Linux, or the Music folder on Windows).
...
1. **Open gamdl-GUI** if it is not already running.
...
Once you have gamdl-GUI set up and running, explore these topics to make the most of the application:
