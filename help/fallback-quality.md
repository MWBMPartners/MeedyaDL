...
- **Safety guard** -- the × button on the only remaining row of a chain is disabled. An empty chain would block every download in that category, so MeedyaDL refuses the last removal.
