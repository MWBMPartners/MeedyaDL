...
**An update never leaves you worse off.** Pressing **Upgrade** on the Updates page never swaps in a copy it happens to find elsewhere on your computer, and only falls back to the MeedyaSuite mirror when the mirror is what the check compared against. If the source cannot be reached, your working copy is left as it is and MeedyaDL tells you. (FFmpeg may still come from its usual download site rather than the mirror; that copy is the same age or newer.) If a new copy of MP4Box turns out not to start, your previous copy is put back.
