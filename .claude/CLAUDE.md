...
- **Fallback quality chains**: Version-aware dual strategy. For GAMDL >= 2.9.1: uses native `--song-codec-priority` flag to pass the entire fallback chain (e.g., `alac,aac,...`) in a single process invocation. For GAMDL < 2.9.1: MeedyaDL's own `try_fallback` system retries with successive codecs on failure. Music chain: ALAC→Atmos→AC3→AacBinaural→Aac→AacLegacy; Video: 2160p→...→240p. Config.ini dual-key strategy writes both `song_codec` (for < 2.9.1) and `song_codec_priority` (for >= 2.9.1) to ensure compatibility across GAMDL versions.
