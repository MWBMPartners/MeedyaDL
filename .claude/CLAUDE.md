...
- **Pre-download checks**: Two blocking gates in `DownloadForm.tsx` before queueing: (1) Internet connectivity check (`check_internet_before_download` — async HTTP GET to apple.com), (2) Cookie validation (`check_cookies_before_download`). Both return `CookieCheckResult { ready, message }` and show amber warning banners when blocked. Internet check runs first; cookie check is skipped for wrapper users.
