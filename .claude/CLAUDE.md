...
- **Pre-download checks**: Two gates in `DownloadForm.tsx` before queueing: (1) Internet connectivity check (`check_internet_before_download` — async HTTP GET to apple.com) — if offline, the download is still queued but auto-start is skipped via `skip_auto_start` param on `start_download`, and a warning toast is shown; (2) Cookie validation (`check_cookies_before_download`) — blocks if cookies expired/missing (skipped when offline). Cookie check is also skipped for wrapper users.
