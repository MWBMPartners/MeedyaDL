...
- **Dependencies**: Self-contained in app data dir (Python via python-build-standalone, GAMDL via pip, tools). Release builds bundle all deps in the installer; on first launch, `bundled_deps_service` extracts them silently.
...
src-tauri/src/          # Rust backend
  commands/             # IPC command handlers (system, dependencies, settings, download [renamed from gamdl], credentials, updates, cookies, login_window, artwork, service_status, smart_download)
  models/               # Data structures (download, settings, gamdl_options, download_options, ytdlp_options, votify_options, get_iplayer_options, dependency, media_service, service_status, content_match)
  services/             # Business logic (python_manager, perl_manager, gamdl_service, service_dispatch, youtube_service, bbc_iplayer_service, spotify_service, dependency_manager [4 required + 3 optional tools], bundled_deps_service, config_service, download_queue, update_checker, cookie_service, login_window_service, animated_artwork_service, apple_music_api, metadata_tag_service, acoustid_service, replaygain_service, service_status, smart_download)
...
src/                    # React frontend
  ...
  stores/               # Zustand state stores (ui, settings, download, dependency, setup, update, serviceStatus)
...
scripts/                # Build utilities (copyright year updater, version bump, download-bundled-deps)
...
- **Dependency manager**: 4 required external tools (FFmpeg, mp4decrypt, N_m3u8DL-RE, MP4Box) + 3 optional (aria2c, fpcalc, amdecrypt). `tool-versions.toml` defines minimum supported versions per tool and mirror config (compiled into binary via `include_str!()`). Download resolution follows a three-tier fallback chain: **System PATH** → **Primary upstream** → **Mirror repo** (`MeedyaDL/MeedyaDL-Tools`) → Error with guidance. `resolve_github_release_asset()` is the generic GitHub API resolver used by both upstream (N_m3u8DL-RE) and mirror queries. `install_tool()` first checks system PATH for a compatible version; if found, copies it to the managed tools dir and writes a `.source` marker file ("system"); otherwise downloads (with mirror fallback) and writes "managed". `DependencyStatus.source` field (Rust + TypeScript) drives the "System" / "Bundled" badges in the setup wizard. MP4Box uses platform-specific installers (macOS Homebrew/`.pkg`, Windows NSIS `.exe`, Linux `.deb`) with mirror fallback via `install_mp4box_with_fallback()`. Mirror assets use standardized naming: `{tool_id}-{os}-{arch}.{ext}`. AcousticID fingerprinting uses the embedded `rusty-chromaprint` library (pure Rust) — fpcalc is bundled as an optional fallback. aria2c provides optional download acceleration (Windows-only upstream binary; macOS/Linux via Homebrew/apt). amdecrypt is an optional wrapper system tool (manual upload to mirror).
- **Bundled dependencies**: Release and pre-release builds bundle all dependencies into the installer via `scripts/download-bundled-deps.sh`: Python + pip packages (gamdl, votify, gytmdl, yt-dlp), binary tools (FFmpeg, mp4decrypt, N_m3u8DL-RE, MP4Box, aria2c, fpcalc, amdecrypt), and Perl + get_iplayer. On first launch, `bundled_deps_service::extract_bundled_deps()` copies them from the app's resource directory to the app data directory, writes `.source` marker files as "bundled", and creates a `.bundled_deps_extracted` marker to prevent re-extraction. The setup wizard auto-advances past completed installation steps (Welcome → Cookies → Done). Dev builds without `src-tauri/bundled-deps/` fall back to the normal download flow. The bundled-deps directory is gitignored. Perl runtime uses `skaji/relocatable-perl` (macOS/Linux); `perl_manager.rs` provides path resolution and status checking for BBC iPlayer's get_iplayer engine.
...
- **Remote service status (kill switch)**: `service-status.json` in repo root (committed to `main`) controls per-service enable/disable and optional messages. Fetched from `raw.githubusercontent.com/MWBMPartners/MeedyaDL/main/service-status.json` on app startup and every 4 hours. Fail-open: network failure → cached config → all-enabled default. Rust service in `services/service_status.rs`, model in `models/service_status.rs`, command in `commands/service_status.rs`. Frontend store `serviceStatusStore.ts`, banner `components/common/ServiceStatusBanner.tsx`. Download queue and DownloadForm both gate on service status. PascalCase service keys in JSON (`AppleMusic`, `YouTube`, `BBCiPlayer`, `Spotify`) mapped to kebab-case `MediaServiceId` in frontend. See `Dev_Notes.md` for developer instructions.
- **Smart Download (cross-platform quality)**: When `smart_download_enabled` is true (default: false), the DownloadForm shows a collapsible panel comparing quality tiers across services. Phase 1: returns only the original service's quality as a baseline. Future: cross-platform search by ISRC/UPC. `QualityTier` enum (Lossy128 → DolbyAtmos) in `models/content_match.rs`. Service quality mapping in `services/smart_download.rs` via `get_service_quality_tier()`. Command in `commands/smart_download.rs`. Toggle in Settings > General.
