...
- **Bundled dependencies**: Release and pre-release builds bundle all dependencies (Python, GAMDL, FFmpeg, mp4decrypt, N_m3u8DL-RE, MP4Box) into the installer via `scripts/download-bundled-deps.sh`. On first launch, `bundled_deps_service::extract_bundled_deps()` copies them from the app's resource directory to the app data directory, writes `.source` marker files as "bundled", and creates a `.bundled_deps_extracted` marker to prevent re-extraction. The setup wizard auto-advances past completed installation steps (Welcome → Cookies → Done). Dev builds without `src-tauri/bundled-deps/` fall back to the normal download flow. The bundled-deps directory is gitignored.
