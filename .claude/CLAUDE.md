...
- **Temp directory**: `temp_path` setting (default: empty string, resolved to `{OS temp dir}/MeedyaDL`). Resolved at runtime in both `merge_options()` and `settings_to_ini()` via `std::env::temp_dir().join("MeedyaDL")`. Avoids GAMDL's default of `.` which is unwritable on macOS from `/Applications`. Configurable in Settings > Paths.
...
- **Dependency manager**: 4 required external tools (FFmpeg, mp4decrypt, N_m3u8DL-RE, MP4Box) + 1 optional (fpcalc for AcousticID). `tool-versions.toml` defines minimum supported versions per tool and mirror config (compiled into binary via `include_str!()`). Download resolution follows a three-tier fallback chain: **System PATH** → **Primary upstream** → **Mirror repo** (`MWBMPartners/meedyadl-tools`) → Error with guidance. `resolve_github_release_asset()` is the generic GitHub API resolver used by both upstream (N_m3u8DL-RE) and mirror queries. `install_tool()` first checks system PATH for a compatible version; if found, copies it to the managed tools dir and writes a `.source` marker file ("system"); otherwise downloads (with mirror fallback) and writes "managed". `DependencyStatus.source` field (Rust + TypeScript) drives the "System" badge in the setup wizard. MP4Box uses platform-specific installers (macOS Homebrew/`.pkg`, Windows NSIS `.exe`, Linux `.deb`) with mirror fallback via `install_mp4box_with_fallback()`. Mirror assets use standardized naming: `{tool_id}-{os}-{arch}.{ext}`.
