...
- **Graceful shutdown**: `ShutdownSignal` in `download_queue.rs` wraps `Arc<AtomicBool>` with `trigger()` / `is_triggered()` methods. Registered as Tauri managed state. Triggered on `WindowEvent::Destroyed` and tray "quit" action. Checked at iteration boundaries in enrichment pipeline (between 5 stages), companion download loops (between tiers), and lyrics companion loops (between formats). Fire-and-forget `tokio::spawn` tasks exit early instead of starting new work after shutdown.
