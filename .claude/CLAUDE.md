...
- **Auto-retry without wrapper**: `auto_retry_without_wrapper` setting (default: `false`). When enabled and a wrapper download fails terminally, the queue automatically re-queues the item with wrapper disabled (cookie-based auth). Toggle in Settings > Advanced > Wrapper section (only visible when `use_wrapper` is true). Logic in `download_queue.rs` terminal error handler calls `retry_without_wrapper()` and re-processes the queue.
